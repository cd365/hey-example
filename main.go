...
func Select() {
	...
	// Plan A for query.
	{
		...
		get := way.Get(m.Table()).Alias(a).Comment("query data plan A").Join(func(j hey.QueryJoin) {
			...
			r1.AddQueryColumns(fmt.Sprintf("b.%s AS %s", m1.NAME, "company_name"))
			r0 := j.GetMaster()
			r0.AddQueryColumns("a.*")
		})
		get.Desc("a.id")
		get.Limit(100)
		way.Debugger(get)
	...
	// Plan B for query, it is strongly recommended to avoid using quoted strings as much as possible.
	{
		...
		get := way.Get(a.Table()).Alias(a.Alias()).Comment("query data plan B").Join(func(j hey.QueryJoin) {
			...
			// r0.AddQueryColumns("a.*")
			r0.AddQueryColumns(a.Column()...)
			r1.AddQueryColumns(tc.Column(b.NAME, "company_name"))
		})
		get.Desc(a.ID)
		get.Limit(100)
		way.Debugger(get)
